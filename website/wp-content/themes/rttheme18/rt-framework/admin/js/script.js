...
jQuery(function($){
	...
	$( ".rt_modal_close" ).on('click', function() { 
	...
	$(document.body).on('click', '.module_add_button', function() {   
		...
		$.post(ajaxurl, data, function(response) {	 
			...
			if( selectedItem === "grid" ){ 
				...
				.hide({duration: 300}).show('scale', { duration: 700, easing: 'swing' });  
...
jQuery(function($){  
	...
	$(document.body).on('click', '.module_close', function() {   
		...
		this_module.hide('fade', { duration: 300, easing: 'swing' }).show('scale', { duration: 300, easing: 'swing' });
