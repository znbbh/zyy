...
$customFields = array(
	...
	foreach($rt_social_media_icons as $key => $value){   
		...
		if ($key=="Email") {
			global $current_user;
			get_currentuserinfo();
			$msgdesc=__(" Link field : Enter a valid URL (http or https) to the members own contact-page [Example : ",'rt_theme_admin'). site_url().__("/contact-",'rt_theme_admin').$current_user->user_login.__("] or emailaddress. [Example : ",'rt_theme_admin').$current_user->user_email.__("] Note : Do not add 'mailto:' as the theme will add that automatically in case a valid emailaddress is used.",'rt_theme_admin');
		}
