	...
	function rt_page_parents(){
		...
		if ( $post->post_parent ){		
			...
			do{				
				...
				$items = rt_create_breadcrumb_item( array( "url"=> get_permalink( rt_wpml_page_id( $parent_page->ID ) ), "title"=> get_the_title( rt_wpml_page_id( $parent_page->ID ) ) ) ) . $items;

				//find parent page of this page
				$parent_page = get_page( $parent_page->post_parent );

			}

			while ( empty( $parent_page->post_parent ) ) ;

		}

		//current page
		$items .= rt_create_breadcrumb_item( array( "title"=> $post->post_title, "rt_delimiter" => "" ) );
...
if( ! function_exists('rt_get_base_page') ){	
	function rt_get_base_page( $for = "" ){
		...
		if( $for == "products" && RT_PRODUCTPAGE != "" ){ 
	 		...
	 		$page = rt_create_breadcrumb_item( array( "url"=> get_permalink( rt_wpml_page_id( RT_PRODUCTPAGE ) ), "title"=> get_the_title( rt_wpml_page_id( RT_PRODUCTPAGE ) ) ) ) ;

	 	}elseif( $for == "portfolio" && RT_PORTFOLIOPAGE != "" ){ 
	 		
	 		$page = rt_create_breadcrumb_item( array( "url"=> get_permalink( rt_wpml_page_id( RT_PORTFOLIOPAGE ) ), "title"=> get_the_title( rt_wpml_page_id( RT_PORTFOLIOPAGE ) ) ) ) ;

	 	}elseif( $for == "blog" && RT_BLOGPAGE != "" ){ 

			$page = rt_create_breadcrumb_item( array( "url"=> get_permalink( rt_wpml_page_id( RT_BLOGPAGE ) ), "title"=> get_the_title( rt_wpml_page_id( RT_BLOGPAGE ) ) ) ) ;
		}
