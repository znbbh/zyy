...
class RTThemeAdmin extends RTTheme{
	...
	function admin_init(){ 
		...
		add_action( 'admin_footer', array(new rt_shortcode_helper, "construct") );
		add_action( 'wp_before_admin_bar_render', array(new rt_shortcode_helper, "custom_toolbar") , 999 );		
		add_filter( 'tiny_mce_version', array(new rt_shortcode_helper, "refresh_editor") );
		add_filter( 'init', array(new rt_shortcode_helper, "rt_theme_shortcode_button") );
	...
	function rt_admin_ajax() {
		...
		if(! isset($_POST['saveoptions'] ) && !isset( $_POST['rt_theme_gallery'] )  && !isset( $_POST['clone'] ) && !isset( $_POST['generateForms'] ) && !isset( $_POST['iconSelector'] ) && !isset( $_POST['new_template'] ) && !isset( $_POST['templateBuilder'] ) && ! isset( $_POST['sidebarCreator'] ) ){ // returns an option box for the template builder			
		...
		}elseif( ! isset( $_POST['saveoptions'] ) && isset( $_POST['rt_theme_gallery'] ) ){ // returns new gallery form
		...
		}elseif( isset( $_POST['iconSelector'] ) ){//icon selection
	...
	function rt_save_options($options){
		...
		$id_array=str_replace("[]","", $id);
			...
			if(isset($_POST[$id_array]) && is_array($_POST[$id_array])){ 
				...
				$request_value = isset( $_POST[ $id ] ) ? stripslashes( $_POST[ $id ] ) : "";
			}
	...
	function rt_generate_forms( $options, $rt_templateID = "") { 
		...
		foreach($options as $k => $v){
			...
			switch ($v['type']){
				...
				#
				case 'textarea';
				echo $label.'<td class="col2"><div class="form_element"><textarea name="'.$id.'" id="'.$css_id.'">'.stripslashes($field_value).'</textarea></div>';
				echo '</td></tr></table>';
