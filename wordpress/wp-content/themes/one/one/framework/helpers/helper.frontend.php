...
if( !function_exists('thb_get_video_thumbnail') ) {
	function thb_get_video_thumbnail( $url, $key='' ) {
		...
		if( $is_youtube || $is_vimeo ) {
			...
			elseif( $is_youtube ) {
				$thumbnail = "http://img.youtube.com/vi/{$code}/maxresdefault.jpg";
			}
		}
		else {
			return '';
...
if( !function_exists('thb_post_has_previous') ) {
	 ...
	 * @return boolean
	 **/
	function thb_post_has_previous( $in_same_cat = false ) {
		global $post;

		$thb_content = '';
		ob_start();
		previous_post_link( '%link', '%title', $in_same_cat );
...
if( !function_exists('thb_post_has_next') ) {
	 ...
	 * @return boolean
	 **/
	function thb_post_has_next( $in_same_cat = false ) {
		global $post;

		$thb_content = '';
		ob_start();
		next_post_link( '%link', '%title', $in_same_cat );
		$thb_content = ob_get_contents();
		ob_end_clean();
...
if( ! function_exists( 'thb_parse_googlefont_import_string' ) ) {
	...
	function thb_parse_googlefont_import_string( $string ) {
		...
		$string = str_replace( '@import url(' . thb_get_protocol() . '://fonts.googleapis.com/css?', '', $string );
...
if( ! function_exists( 'thb_video' ) ) {
	...
	function thb_video( $url, $options = array() ) {
		...
		$is_self_hosted = $is_mp4 || $is_ogv || $is_mov;
		...
		if ( $holder ) {
			...
			if ( $is_self_hosted ) {
				...
				if ( $is_youtube ) {
					...
					printf( '<iframe class="thb-video-api thb-video-youtube" id="%s" type="text/html" src="https://www.youtube.com/embed/%s?enablejsapi=1&modestbranding&showinfo=0%s" frameborder="0"></iframe>', $id, $code, $params );
				}
				elseif( $is_vimeo ) {
					if ( $loop ) {
						$params .= '&loop=1';
					...
					printf( '<iframe class="thb-video-api thb-video-vimeo" id="%s" src="//player.vimeo.com/video/%s?player_id=%s&badge=0&byline=0&title=0&color=fff%s" frameborder="0"></iframe>', $id, $code, $id, $params );
				}
				else {
					global $wp_embed;
					echo $wp_embed->run_shortcode('[embed]' . trim( $url ) . '[/embed]');
