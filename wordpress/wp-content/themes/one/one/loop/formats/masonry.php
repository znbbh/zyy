...
<div class="<?php echo $wrapper_classes; ?>">
	...
	<div class="loop-post-content">
			...
			<header class="item-header">
				<?php if ( !empty( $thb_title ) || !empty( $thb_title_alt ) ) : ?>
					<?php if( $thb_format === 'quote' ) : ?>
						<h1 <?php thb_livestyle_element( 'loop_header_quote_masonry' ); ?>>
							<a href="<?php the_permalink(); ?>" rel="permalink">
								<?php echo $thb_title_alt; ?>
							</a>
						</h1>
					<?php else : ?>
						<h1 <?php thb_livestyle_element( 'loop_header_masonry' ); ?>>
							<a href="<?php the_permalink(); ?>" rel="permalink">
								<?php echo $thb_title; ?>
							</a>
						</h1>
					...
					<div class="loop-post-meta" <?php thb_livestyle_element( 'loop_post_meta_masonry' ); ?>>
						<ul>
							<?php if( thb_author_block_enabled( $thb_post_id ) ) : ?>
								...
								<li><?php echo get_avatar( get_the_author_meta( 'ID' ), 30 ); ?> <?php _e('by', 'thb_text_domain'); ?> <?php the_author_posts_link(); ?></li>
		...
		<?php if( $thb_format != 'quote') : ?>
			...
			<?php if( $thb_format != 'aside') : ?>
				<a class="thb-btn thb-read-more" href="<?php the_permalink(); ?>" rel="permalink">
