...
$thb_tab = new THB_Tab( __('General', 'thb_text_domain'), 'general' );
	...
	$thb_container = $thb_tab->createContainer( __('General options', 'thb_text_domain'), 'general_options' );
		...
		if ( thb_config( 'backpack/general', 'copyright' ) ) {
			...
			$help = 'The copyright text will be displayed at the bottom of the site (Note: accepts basic HTML).';

			if ( defined( 'QT_SUPPORTED_WP_VERSION' ) ) {
				$thb_field = new THB_TextareaField('copyright');
				$help = 'The copyright text will be displayed at the bottom of the site (Note: accepts basic HTML).<br><br>To translate the message, one per line, prepend the locale of the language. E.g. <code>de_DE:your translated string</code>.';
			}

			$thb_field->setLabel( __( 'Copyright text', 'thb_text_domain' ) );
			$thb_field->setHelp( __( $help, 'thb_text_domain' ) );
			$thb_container->addField($thb_field);
		}

		if ( thb_config( 'backpack/general', 'rss' ) ) {
